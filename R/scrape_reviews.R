...
scrape_reviews <- function(asin, page_number) {
  base::paste0("https://www.amazon.it/product-reviews/",asin,"/?pageNumber=", page_number) %>%
    xml2::read_html() ->
    doc
  ...
  data.frame(title = (review_title %>%
                        base::substring(19, base::nchar(review_title)-4)),
             text = (review_text %>%
                       base::substring(19, base::nchar(review_text)-4)),
             star = (review_star %>%
                       base::substring(1, 1))
             ) %>%
    base::return()
