...
get_asin <- function(url) {
  first_attempt <- xml2::read_html(url) %>%
    rvest::html_node("[id='ASIN']") %>%
    rvest::html_attr("value")
  if (is.na(first_attempt)) {
    xml2::read_html(url) %>%
      rvest::html_node("[name='ASIN']") %>%
      rvest::html_attr("value") %>%
      return()
  }
  else {
...
number_of_reviews <- function(asin){
  base::paste0("https://www.amazon.it/product-reviews/", asin, "/ref=cm_cr_arp_d_viewopt_sr?filterByStar=critical&pageNumber=1") %>%
    xml2::read_html() %>%
    rvest::html_node("[data-hook='cr-filter-info-review-count']") %>%
    rvest::html_text() %>%
    stringr::str_extract("recensioni su (?s)(.*)") %>%
    base::substring(15,) %>%
    base::gsub('\\.','', .) %>%
    base::as.numeric() %>%
    return()
...
acquire_reviews <- function(asin, page_num){
  base::paste0("https://www.amazon.it/product-reviews/",asin,"/ref=cm_cr_arp_d_viewopt_sr?filterByStar=critical&pageNumber=",page_num) %>%
    xml2::read_html() ->
    doc
  ...
  data.frame(title = (review_title %>% base::substring(19,nchar(review_title)-4)),
             text = (review_text %>% base::substring(19,nchar(review_text)-4)),
             star = (review_star %>% base::substring(1, 1))) %>%
    return()
}
